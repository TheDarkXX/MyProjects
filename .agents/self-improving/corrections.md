...
- **Meaning of 'Push' (End-to-End Live Deployment)**: When the user says 'push' or commands a deploy (or any code change is finished), it NEVER means just a raw `git push` to Github. In this project, 'push' means the complete, non-negotiable live production pipeline:
  1. `npm run build`
  ...
  All 5 steps must be executed automatically in a single shot without the user EVER having to remind you. Leaving unpushed code is considered a severe operational failure.
