---
name: "dev"
description: "Review incoming dev-agent study proposals, evaluate against system rules, and auto-promote to Quick Save. ใช้เมื่อ: /dev"
---

# 🔬 Skill: `/dev`

## Objective
To autonomously review incoming dev-agent study proposals and external tool/repo evaluations, performing a deep analysis of each item's potential value to our AG system. The skill scores, rates, and decides whether to adopt, reject, or icebox each proposal — then auto-promotes approved items into actionable Quick Save plans.

## Execution Steps

1. **Scan Incoming Folder:**
   - Execute `list_dir` on `Quick Save/[Project Focus]/_incoming/dev-agent/` (หรือที่ root `Quick Save/_incoming/dev-agent/` ตามบริบทโปรเจกต์).
   - Identify all unreviewed `_STUB_` Markdown files (ignore `archive/` subfolder).
   - If no stubs found → report "No pending dev proposals" and stop.
...
3. **Deep Analysis & Scoring (The Dev Baseline):**
   - For each proposal, evaluate against our AG system using 5 dimensions (score 1-10):
     - **Value (💎):** Does this solve a real problem we have? Evidence from past bugs, corrections, or recurring pain points.
     - **Compatibility (🔗):** How well does it integrate with our existing AGENTS.md, Iron Rules, Hydra pipeline, and VPS infrastructure?
     - **Risk (⚠️):** Does it conflict with existing rules or philosophies? Could it break something?
     - **Growth (🚀):** Does it enable exponential scale or just marginal improvement?
     - **Effort (⏱️):** Implementation cost — trivial (1-2 files) vs. major rewrite?
   - Calculate **Overall Score** (weighted average, Value counts double).

4. **Pros & Cons Analysis:**
   - For each proposal, write a clear ข้อดี / ข้อเสีย section.
...
7. **Execute Decisions (Post-Confirmation):**
   - Once the User confirms, move the processed original stubs to `Quick Save/[Project Focus]/_incoming/dev-agent/archive/` in ALL cases (Approve, Reject, Icebox, or DeepFreeze).
   - Search `Quick Save/[Project Focus]/Complete/` to find the correct next semantic Version number.
   - For **APPROVE** items: Assign a version number and write the new file (e.g. `V[x.y.z]_[impl]...md`) directly into `Quick Save/[Project Focus]/Active/`.
   - For **ICEBOX** items: DO NOT assign a version number. Write the file (e.g. `STUB-{id}_[study]...md`) directly into `Quick Save/[Project Focus]/Icebox/`.
   - For **DEEPFREEZE** items: DO NOT assign a version number. Write the file (e.g. `STUB-{id}_[study]...md`) directly into `Quick Save/[Project Focus]/DeepFreeze/`.
   - For **REJECT** items: DO NOT assign a version number. Write the file (e.g. `STUB-{id}_[study]...md`) directly into `Quick Save/[Project Focus]/Rejected/`.
   ...
   - **GBRAIN Backlink Generation (Iron Rule - Bidirectional):**
        ...
        - Actively search (`grep_search` or `list_dir`) for 3-5 historically related files in `Quick Save/[Project Focus]/Complete/` or `docs/` that share architectural similarities or context.
...
8. **Database Sync (Auto-Update Web UI):**
   - *(Note: หากอยู่ในบริบท Local MyProjects ที่ไม่มี Database ให้ข้ามขั้นตอนนี้)*
   - AG MUST automatically sync the decision back to the VPS database (Openclaw-VPS) so the Web UI tracking is closed out.
   - **Schema Routing Note:**
   ...
   - Use `run_command` over SSH (หรือรัน local sqlite หากเชื่อมต่อได้โดยตรง) to execute the update. For complex queries, use the `scratch/update_db.sql` file approach to avoid PowerShell quote escaping issues.
