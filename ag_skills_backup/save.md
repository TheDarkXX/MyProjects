...
3. **Apply Compiled Truth + Timeline Pattern.**
4. **Verify Full-Detail Rule:** Do NOT summarize.
...
4. **Write file** to `MyProjects/Quick Save/Active/[ProjectName]/` (or `Complete/` if done).

### Phase 2: Openclaw-VPS Sync (ศูนย์บัญชาการ)
5. **Copy** the same file to `c:\My Claw\Openclaw-VPS\Quick Save\Active\DoctorBank-Band\` (or respective project folder in Openclaw).
6. **Update Openclaw Manifest:** Run `node scripts/qs-indexer.js --incremental` in Openclaw-VPS.

### Phase 3: Git (MyProjects repo)
7. **Commit:** `git add .` + `git commit -m "[AG] Save [ProjectName] [version]"` in MyProjects root.
8. **Push:** `git push origin` (sync between PCs).
9. **Report:** Done.
