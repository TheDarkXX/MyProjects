...
#### 2.3 Universal Adaptive Logo Engine & Proportional Typography
ปฏิบัติตามกฎเหล็กของ AGENTS.md (ฟอนต์ห้ามต่ำกว่า 13px ในกล่องหลัก และคอนทราสต์ $\ge 70\%$ พร้อมระบบคำนวณ Responsive Scale ตามพื้นที่จริง `minDim = Math.min(width, height)`):
- **Giant Tiles** ($minDim \ge 130\text{px}$, เช่น NVDA, AAPL, GOOGL, MSFT, TSLA, WMT):
  - โลโก้ขยายเต็มตาระดับ **52px – 56px** (`Math.round(minDim * 0.24)`)
  - Ticker Symbol: **`text-[22px]` – `text-[24px]` font-bold** สีขาวสว่าง `#FFFFFF`
  - Change %: **`text-[16px]` – `text-[18px]` font-bold** สีขาวสว่าง `#FFFFFF`
- **Large Tiles** ($minDim \ge 85\text{px}$, เช่น AVGO, AMD, COST, UNH, XOM):
  - โลโก้ขนาด **32px – 38px**
  - Ticker Symbol: **`text-[16px]` font-bold**
  - Change %: **`text-[14px]` font-semibold**
- **Medium Tiles** ($w \ge 48\text{px} \land h \ge 46\text{px}$, เช่น ORCL, CSCO, PLTR, CRM, IBM, NFLX, UBER, KO, PG, WFC, CAT, RTX, GE):
  - โลโก้ขนาด **20px – 26px** (ใส่โลโก้ครบทุกตัว ไม่ตัดทิ้ง)
  - Ticker Symbol: **`text-[13px]` font-bold**
  - Change %: **`text-[12px]` font-semibold**
- **Small Tiles** ($w \ge 34\text{px} \land h \ge 34\text{px}$, เช่น PANW, CRWD, FTNT, ADBE, INTU, AMGN, VRTX, ISRG, GILD, LRCX, AMAT):
  - โลโก้ขนาด **16px – 18px**
  - Ticker Symbol: **`text-[12px]` font-bold**
  - Change %: **`text-[11px]` font-semibold**
- **Micro Tiles** ($w < 34\text{px} \lor h < 34\text{px}$):
  - โชว์โลโก้จิ๋ว 16px หากพื้นที่พอ ($w, h \ge 26\text{px}$) หรือรหัสย่อ 3 ตัวอักษร

#### 2.4 Parqet Logo CDN with 2-Tier Fallback Engine (`StockLogo.tsx`)
- เนื่องจาก Clearbit ปิดบริการ Public DNS (`logo.clearbit.com` เกิด `ENOTFOUND`) ระบบจึงเปลี่ยนมาใช้ **Parqet Stock Logo CDN** เป็นตัวหลัก:
  - **Primary CDN:** `https://assets.parqet.com/logos/symbol/${cleanSymbol}?format=png` (ดึงโลโก้จริงระดับ HD: ตาเขียว NVDA, Apple ขาวดำ, Google 4 สี, Microsoft 4 สี, Meta ฟ้า)
  - **Secondary Fallback:** `https://t1.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=https://${domain}&size=128` (Google Favicon V2 อัปไทม์ 100%)
  - **Final Fallback:** วงกลม Monogram ตัวย่อ 2 ตัวอักษรพร้อมพื้นหลัง `#262B3E`
- ระบบ In-memory Cache (`failedParqet`, `failedGoogle`) ป้องกันการยิง Network ซ้ำซ้อนอย่างมีประสิทธิภาพ
...
- **12:40**: ผู้ใช้ตรวจหน้าจอ Live Deploy ชี้ว่าสีเป๊ะมาก แต่ตรวจพบ 2 จุดสำคัญ: 1) Sector Name ไม่แสดง 2) Logo หุ้นไม่ขึ้น เป็นตัวย่อ Monogram 2 อักษรหมด และขอให้ออกแบบบรีฟอย่างละเอียด
- **12:42**: เจาะลึกโค้ดพบต้นตอ 2 จุด:
  1. D3 Treemap Layout มีการเรียก `.paddingTop(26)` แล้วตามด้วย `.paddingOuter(3)` ซึ่งฟังก์ชัน `paddingOuter` ของ D3 ไปทับค่า `paddingTop` กลายเป็น 3px ส่งผลให้แผ่นหุ้นเริ่มต้นวางที่ `top: 3px` แล้วทับแถบ Sector Header ข้างหลังมิดสนิท 100%
  2. Clearbit Public DNS (`logo.clearbit.com`) ถูกปิดบริการอย่างเป็นทางการ ทำให้เกิด `getaddrinfo ENOTFOUND` รีเควสต์ล้มเหลวทุกตัวและตกไปที่ Error Monogram
- **12:45**: ค้นพบและทดสอบ Parqet Stock Logo CDN (`https://assets.parqet.com/logos/symbol/${symbol}?format=png`) พบว่าแสดงผลโลโก้จริงคมชัดระดับ HD แบบเดียวกับ TradingView (Apple ขาวดำ, Nvidia ตาเขียว, Google 4 สี, Microsoft 4 สี) พร้อมเสริม Google Favicon V2 เป็น Secondary Fallback
- **13:03**: ผู้ใช้อนุมัติ "ลุยเลย" ดำเนินการปรับแก้ `heatmapEngine.ts` (สลับและใช้ฟังก์ชัน `.paddingTop((node) => node.depth === 1 ? 24 : 2)`), ปรับ `MarketHeatmap.tsx` ให้ Sector Header อยู่แถบบน `h-[22px]` ด้วยสี `#D1D4DC` และจัด Layout แผ่นหุ้นแบบ TradingView (Logo + Ticker + Percent), ปรับ `StockLogo.tsx` เป็น Parqet + Google Favicon V2 Fallback
- **13:05**: บิลด์ `npm run build` สำเร็จ 100% (11.67s), รัน `npm run bump-cache` ได้เวอร์ชัน `v=20260910_130545` พร้อมเตรียม Deploy ขึ้น VPS เรียบร้อย
- **13:10**: ผู้ใช้ตรวจสอบเทียบกับ TradingView พบว่า 1) ตัวใหญ่ Logo และ Font เล็กเกินไปเมื่อเทียบกับ TradingView 2) พอเป็นหุ้นขนาดกลางและเล็ก (ORCL, CSCO, PLTR, CRM, KO, PG ฯลฯ) โลโก้หายเกลี้ยงเพราะโค้ดเดิมไม่ได้ใส่ `<StockLogo />` ใน Tier M/S 3) ผู้ใช้ยืนยันชอบ Hover แบบลอย (Rich Card) ของระบบมากกว่า ให้คงไว้ตามเดิม
- **13:15**: พัฒนาระบบ Universal Adaptive Logo Engine ใน `MarketHeatmap.tsx` คำนวณขนาด Logo และ Font อัตโนมัติตามสัดส่วนพื้นที่จริง `minDim = Math.min(w, h)`:
  - Giant (≥ 130px): Logo 52–56px, Ticker 22–24px font-bold, Percent 16–18px font-bold (NVDA, AAPL, GOOGL, MSFT, TSLA)
  - Large (85–129px): Logo 32–38px, Ticker 16px font-bold, Percent 14px font-semibold (AVGO, AMD, COST, UNH, XOM)
  - Medium (48–84px): ใส่ Logo 20–26px, Ticker 13px, Percent 12px (ORCL, CSCO, PLTR, CRM, NFLX, KO, PG, WFC, CAT, RTX, GE)
  - Small (34–47px): ใส่ Logo 16–18px, Ticker 12px (PANW, CRWD, FTNT, ADBE, AMGN, VRTX, ISRG, GILD, LRCX, AMAT)
  - Micro (< 34px): Logo 16px หรือ 3-char Ticker
- **13:16**: บิลด์ `npm run build` สำเร็จ 100% (13.49s), รัน `npm run bump-cache` ได้เวอร์ชัน `v=20260910_131600`, อัปโหลด SCP ขึ้น VPS `185.250.38.247`, รีโหลด PM2 `stock-api` และพุชโค้ดขึ้น GitHub เรียบร้อย
...
- **2026-09-10 16:35** | [Tiger Portfolio 20-Year Dynasty Playbook (V2.15.1)](file:///c:/My%20Claw/MyProjects/Quick%20Save/Complete/My%20Stock%20Portfolio/V2.15.1_%5Bdesign%5D_strategy_tiger-portfolio-project-2x-hybrid-20yr-playbook.md) -- แผนยุทธศาสตร์ Project 2X Hybrid 5 จตุรเทพ และโมเดลจำลองทบต้น 20 ปี
- **2026-09-10 19:30** | [X-Chart USD/THB 4H Timeframe, 2-Year Engine & Settings Toggle (V2.16.0)](file:///c:/My%20Claw/MyProjects/Quick%20Save/Complete/My%20Stock%20Portfolio/V2.16.0_%5Bimpl%5D_ui_xchart-usd-thb-4h-timeframe-and-preferences-toggle.md) -- การพัฒนาไทม์เฟรม 4H ของ USD/THB ใน X-Chart ควบคู่กับการสลับ Resolution บน Lightweight Charts
