...
- **Issue 8:** Bullets in Executive Summary collapsed into a single continuous block of text; AI Priority Reason box sat above article content, obstructing reading flow.
  - **Fix:** Built `normalizedSummary` preprocessor in `NewsDetailModal.tsx` to decompose Unicode `•` and raw text breaks into true Markdown unordered list items (`- `), rendering distinct vertical `<li>` rows with glowing amber dots (`#FFA657`) and generous spacing (`space-y-3`). Swapped section hierarchy to render `📌 บทสรุปและบริบทข่าว (Executive Summary)` first and `💡 เหตุผลที่คัดเกรด (AI Priority Reason)` at the bottom before tags. Bumped CSS cache buster to `v=20260916_191300`, rebuilt, deployed live to VPS, and reloaded PM2.
- **Issue 9:** Dummy placeholder stubs with developer debug text (`หมายเหตุ: ข่าวสารระดับกลาง... บันทึกเฉพาะหัวข้อโดยไม่เรียก AI สรุปเพื่อประหยัดทรัพยากร`) appeared in the feed for `TITLE_ONLY` route articles.
  - **Root Cause:** `newsRadar.js` route for `TITLE_ONLY` (triage score 30-69) inserted hardcoded empty stubs into `news_intelligence` instead of dropping them or performing full synthesis. This polluted the feed with 2 newsletter stubs (ALAB id 3, MARKET id 5).
  - **Fix:** Purged the 2 dummy articles from VPS SQLite database (clean count: 116 verified articles). Refactored `newsRadar.js` to eliminate inserting `TITLE_ONLY` articles into `news_intelligence` entirely (they remain only in `seen_articles` for deduplication audit). Deployed updated `newsRadar.js` to VPS and reloaded PM2 `stock-api`.
- **Issue 10:** Clickbait 13F whale gossip ("มหาเศรษฐี ทิ้ง NVDA, AVGO, LRCX เกลี้ยงพอร์ต...") scored 100 pts and was misclassified as `CATALYST` (Tier 2).
  - **Root Cause:** (1) In `triageArticle()`, holding NVDA (+80) and matching AVGO (+60) pushed score to 140 before subtraction, so even with noise penalty (-40) it still clamped to 100; (2) `NOISE_KEYWORDS` lacked Thai clickbait patterns (`มหาเศรษฐี`, `เกลี้ยงพอร์ต`, `ขายหมดพอร์ต`, `อัดเงินซื้อ`, `เซียน`); (3) `migrate_to_4_tiers.js` defaulted all held stocks to CATALYST unless matched by English keywords, completely ignoring the fact that the AI's own `priority_reason` explicitly stated "เป็นการปรับพอร์ตของนักลงทุนรายหนึ่ง ไม่ใช่เหตุการณ์ที่กระทบปัจจัยพื้นฐานหรือความได้เปรียบการแข่งขันของ NVDA โดยตรง".
  - **Fix:** Added Thai clickbait terms to `NOISE_KEYWORDS`; implemented hard cap in `triageArticle()` ensuring any noise article is clamped to max 35 pts; reinforced guardrail in `synthesizeWithAI()` to auto-demote any article matching noise or whose priority_reason mentions no fundamental impact to `CHATTER`. Executed SQLite reclassification script on VPS to demote ID 1, 2, 42, 44, 45, 46 to `CHATTER` (35 pts), deployed updated `newsRadar.js`, and reloaded PM2.
...
- **2026-09-17 02:30** | [V2.33.1 Date Sort Investigation](file:///c:/My%20Claw/MyProjects/Quick%20Save/Complete/My%20Stock%20Portfolio/V2.33.1_[support]_news-intel_date-sort-investigation.md) -- Clarified date DESC sorting behavior.
