...
related:
  - "lib/hydra-memory.js"
summary: >
  Iceboxed EVO-421 (Structured Evals & Span Tracing). Solid concept but premature execution. Adding span tracing to `ai-call.js` hot path is risky for marginal observability gains. We should consider this once we have a dedicated telemetry service, not by bolting onto the critical AI gateway. Revisit in 1-3 months.
---
# Integrate Structured Evals & Span Tracing for Agent Feedback Loop

## 📌 Context (Compiled Truth)
- **Idea**: Add eval-runner.js with structured metrics and span tracing in `ai-call.js`.
- **Verdict**: ⏸️ ICEBOX
- **Scores**: Impact ⭐⭐⭐, Feasibility ⭐⭐, Urgency ⭐, Alignment ⭐⭐⭐. Overall 2.25/5.
- **Reasoning**: Premature for the hot path. Needs a dedicated telemetry service first.
...
- **2026-04-30**: AG /evo review decided to Icebox. Good architecture idea but wrong timing. We'll hold until a separate telemetry service is in scope.

## 🔗 GBRAIN Backlinks
- **2026-04-30 18:05** | [Hydra Pre-deploy Health Check](file:///c:/My%20Claw/Openclaw-VPS/Quick%20Save/Complete/V8.2.0_[impl]_hydra_pre-deploy-health-check.md) -- related to pre-deploy validation.
