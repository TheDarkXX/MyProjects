...
---
# pyJianYingDraft SDK Reference for Hypercut (Partial Approval)

## 📌 Context (Compiled Truth)
The VectCutAPI proposal was evaluated. The main Cloud API and MCP integration were ICEBOXED because VVE is currently focused on Phase 5 (Scene & B-Roll Generator) and adopting a cloud API now would split focus. Furthermore, VectCutAPI lacks a direct rendering API.
However, the internal `pyJianYingDraft` Python SDK was **APPROVED** as a partial implementation reference. Our current `hypercut` skill relies on manual JSON manipulation of `draft_content.json`, which is fragile and error-prone. The `pyJianYingDraft` SDK provides structured Python classes for manipulating CapCut drafts locally. We will study its architecture to improve our `hypercut` JSON injection pipeline, transitioning away from fragile string replacements to object-oriented draft updates.
