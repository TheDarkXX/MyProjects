...
export const useBackupAndRestore = ({ setNotification, portfolios }: BackupPageProps) => {
  ...
  const fetchAppCheckpoints = useCallback(async () => {
      ...
      if (error) {
          setNotification({ message: `Failed to fetch app checkpoints: ${error.message}`, type: 'error' });
      } else {
  ...
  const deleteAppCheckpoint = async () => {
    ...
    try {
        ...
        setNotification({ message: 'App checkpoint deleted.', type: 'success' });
        await logActivity('app_checkpoint_delete', { summary: states.appCheckpointInModal.version_summary });
        fetchAppCheckpoints();
    } catch (error) {
        const err = error as Error;
        setNotification({ message: `Failed to delete app checkpoint: ${err.message}`, type: 'error' });
    } finally {
  ...
  const exportAppCheckpoint = (checkpoint: AppCheckpoint) => {
    ...
    setNotification({ message: `Exported ${fileName}`, type: 'success' });
  };
  ...
  const fetchCloudBackups = useCallback(async () => {
      ...
      if (error) {
          setNotification({ message: `Failed to fetch cloud backups: ${error.message}`, type: 'error' });
  ...
  const restoreFromCloud = async () => {
      ...
      try {
          ...
          setNotification({ message: 'Restore from cloud complete! App is refreshing...', type: 'success' });
          ...
          setNotification({ message: `Cloud restore failed: ${err.message}`, type: 'error' });
      } finally {
  ...
  const deleteCloudBackup = async () => {
    ...
    try {
        ...
        if (error) throw error;
        setNotification({ message: 'Cloud backup deleted.', type: 'success' });
        ...
        const err = error as Error;
        setNotification({ message: `Failed to delete cloud backup: ${err.message}`, type: 'error' });
    } finally {
  ...
  const executeDataImport = async (backupData: { data: { portfolios: any[], transactions: any[] } }) => {
    ...
    const transactionsToInsert = backupData.data.transactions
        .map(({ id, ...t }: any) => {
            ...
            const newT = { ...t, portfolioId: newPortfolioId || '' };
            delete newT.portfolio_id; // Clean up old key if it exists
            return newT;
        })
  ...
  const importLocalBackup = async (file: File) => {
    ...
    try {
        ...
        setNotification({ message: 'Import complete! App is refreshing...', type: 'success' });
  ...
  const mergeImportLocalBackup = async () => {
    ...
    try {
      ...
      const transactionsToInsert = backupData.data.transactions
        ...
        .map(({ id, ...t }: any) => {
            const newT = { ...t, portfolioId: targetPortfolioId };
            delete newT.portfolio_id;
            return newT;
        });

      await logActivity('merge_local_start', { target_portfolio: targetPortfolio.name, file: file.name });
      
      const { error: deleteError } = await supabase.from('transactions').delete().eq('portfolioId', targetPortfolioId);
      ...
      setNotification({ message: `Successfully replaced transactions for ${targetPortfolio.name}! App is refreshing...`, type: 'success' });
  ...
  const exportLocalBackup = async () => {
    ...
    try {
        ...
        const backupPayload = {
            metadata: { app_name: "InvestTrack AI", created_at: new Date().toISOString(), summary: { portfolios: portfolios?.length, transactions: transactions?.length } },
            data: { portfolios, transactions }
        };
        const now = new Date();
        ...
        setNotification({ message: `Backup "${fileName}" created successfully.`, type: 'success' });
    } catch (error) {
        const err = error as Error;
        ...
        <div className="bg-[#111827] rounded-lg shadow-2xl p-6">
            ...
            <div className="space-y-4">
                {cloudBackups.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">No cloud backups found yet. Make a transaction to trigger the first one.</div>
                ) : (
...
const AppCheckpointRestoreModal: React.FC<{ checkpoint: AppCheckpoint | null, onClose: () => void, setNotification: BackupPageProps['setNotification'] }> = ({ checkpoint, onClose, setNotification }) => {
    ...
    const copyToClipboard = async () => {
        try {
            ...
            setNotification({ message: 'Failed to copy code.', type: 'error' });
        }
    };
  ...
  return (
    <div className="container mx-auto p-4 md:p-6 text-white space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400 mt-2">Manage automatic cloud backups, local data files, and application code checkpoints.</p>
      </div>
