...
export type ScorecardLayoutMode = '3col' | 'panoramic' | 'stacked';

export interface ScorecardPageProps {
  showHeader?: boolean;
  layoutMode?: ScorecardLayoutMode;
  onLayoutChange?: (mode: ScorecardLayoutMode) => void;
}

export const ScorecardPage: React.FC<ScorecardPageProps> = ({
  showHeader = true,
  layoutMode: propLayoutMode,
  onLayoutChange: propOnLayoutChange,
}) => {
  ...
  const [internalLayoutMode, setInternalLayoutMode] = useState<ScorecardLayoutMode>(() => {
  ...
  const handleLayoutChange = (mode: ScorecardLayoutMode) => {
    if (propOnLayoutChange) {
      propOnLayoutChange(mode);
    } else {
