...
export const ValuationCorridorChart: React.FC<ValuationCorridorChartProps> = ({
  ...
  if (!chartData) {
    return (
      <div className="bg-[#12162B]/95 p-4 rounded-2xl border border-white/10 shadow-xl">
  ...
  return (
    <div className="bg-[#12162B]/95 p-4 rounded-2xl border border-white/10 shadow-xl backdrop-blur-md">
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          <span className="px-2.5 py-0.5 rounded-full bg-violet-950/70 text-violet-200 border border-violet-800/60 text-[12px] font-mono font-medium">
        ...
        >
          <defs>
            <linearGradient id={`corridorBand_${symbol}`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#823AFD" stopOpacity="0.30" />
              <stop offset="100%" stopColor="#12162B" stopOpacity="0.05" />
            </linearGradient>
          </defs>

          <g transform={`translate(${margin.left}, ${margin.top})`}>
            {/* Mean Band Shading (Electric Violet) */}
            ...
            {/* Forward PE Line (Burnt Orange Dashed) */}
            {chartData.fwdLine && (
              <path d={chartData.fwdLine} fill="none" stroke="#FD5514" strokeWidth={1.8} strokeDasharray="3,3" opacity={0.85} />
            )}

            {/* Trailing PE Line (Electric Violet) */}
            <path d={chartData.peLine} fill="none" stroke="#823AFD" strokeWidth={2.4} />

            {/* Latest PE Dot */}
            {hoverIdx === null && (
              <>
                <circle cx={chartData.lastX} cy={chartData.lastY} r={4.5} fill="#FFFFFF" stroke="#823AFD" strokeWidth={2.5} />
            ...
            {hoverIdx !== null && peHistory[hoverIdx] && (
              <g>
                {(() => {
                  ...
                  return (
                    <>
                      <line x1={hX} x2={hX} y1={0} y2={innerH} stroke="#823AFD" strokeDasharray="2,2" strokeWidth={1} />
                      <circle cx={hX} cy={chartData.lastY} r={5} fill="#823AFD" stroke="#FFFFFF" strokeWidth={2} />
                      <rect
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/10 text-sm">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5 font-medium">
            <span className="w-3 h-1 rounded bg-[#823AFD] inline-block shadow-sm" />
            <span className="text-slate-300">Trailing PE</span>
          </span>
          <span className="flex items-center gap-1.5 font-medium">
            <span className="w-3 h-1 rounded bg-[#FD5514] inline-block" style={{ borderTop: '1px dashed' }} />
            <span className="text-slate-300">Forward PE</span>
          </span>
        </div>
        <div className="flex items-center gap-2 font-mono text-sm font-semibold">
          <span className="text-slate-400">Band:</span>
          <span className="text-violet-300">{chartData.lowerBand}x</span>
          <span className="text-slate-500">–</span>
          <span className="text-[#FC2D79]">{chartData.upperBand}x</span>
        </div>
      </div>
    </div>
  );
