...
const PerformanceChartPage: React.FC<PerformanceChartPageProps> = () => {
    ...
    const calculateTwr = useCallback((portfolioId: string) => {
        ...
        for (let d = new Date(firstTxDate); d <= lastPriceDate; d.setDate(d.getDate() + 1)) {
            ...
            performanceData.push({
                ...
                qqqPrice: findPrice(dateStr, 'QQQ', lastKnownPrices['QQQ']),
    ...
    useEffect(() => {
        ...
        const baseQqq = baseDataPoint.qqqPrice;
        ...
        const processedData = dataInRange.map(d => {
            ...
            const qqqReturn = (baseQqq && d.qqqPrice) ? ((d.qqqPrice / baseQqq) - 1) * 100 : undefined;
    ...
    useEffect(() => {
        ...
        if (visibleData.length > 0) {
            ...
            const rebasedData = visibleData.map(d => ({
                ...
                'Nasdaq 100': d['Nasdaq 100'] !== undefined && baseline['Nasdaq 100'] !== undefined ? d['Nasdaq 100'] - baseline['Nasdaq 100'] : undefined,
    ...
    const lineColors = { 
        'My Portfolio': '#FFC300', 
        'S&P 500': '#9013FE', 
        'Nasdaq 100': '#06B6D4', // Vibrant Cyan for Tech Hurdle
        'SCHG': '#F5A623',
    ...
    const handleExportExcel = () => {
        const dataToExport = displayData.map(d => ({
            ...
            'Nasdaq 100 (%)': d['Nasdaq 100']?.toFixed(4),
    ...
    return (
        <div className="w-full h-full min-h-[calc(100vh-80px)]" style={{ fontFamily: "'Manrope', 'IBM Plex Sans Thai', system-ui, sans-serif" }}>
