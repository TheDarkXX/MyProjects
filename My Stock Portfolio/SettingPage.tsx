...
import { useBackupAndRestore } from './BackupPage';
...
const testPolygonApiKey = async (key: string): Promise<boolean> => {
    ...
    try {
        ...
        // A successful response for aggregates usually has a "status" of "OK"
...
const SettingPage: React.FC<SettingPageProps> = ({ 
    apiStatus, setApiStatus, dbError, portfolios, setNotification
}) => {
  ...
  // --- Local Backup State & Logic (Moved from BackupPage) ---
  const manager = useBackupAndRestore({ setNotification, portfolios });
  const { isExportingLocal, isImportingLocal, lastError, importLocalBackup, handleMerge } = manager;
  ...
  const handleRestoreClick = () => {
      if(!selectedFile) {
          setNotification({ message: 'Please select a backup file first.', type: 'error' });
          return;
      }
      if (restoreMode === 'full') {
          ...
          if (targetPortfolioId) {
              ...
              setNotification({ message: 'Please select a target portfolio for the merge.', type: 'error' });
          }
      }
  ...
  useEffect(() => {
    ...
    if (savedPolygonTimestamp) setLastUpdatedPolygon(savedPolygonTimestamp);
  }, []);
        ...
        <div className="space-y-6">
            <div className="bg-[#111827] rounded-lg shadow-2xl p-6">
             ...
             <div className="bg-[#111827] rounded-lg shadow-2xl p-6">
                ...
                {lastError && <div className="bg-red-900/50 text-red-200 p-3 rounded-md mb-4 text-sm"><strong>Error:</strong> {lastError}</div>}
